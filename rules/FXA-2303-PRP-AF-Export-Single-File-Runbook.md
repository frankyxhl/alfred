...
| 2026-06-12 | v1.1 (CHG-2304, first real use): repeatable --source/--type + --include project files — delta panel [deepseek 9.8 PASS, minimax 9.5 PASS; GLM slot skipped for the ~80-line delta on a triple-PASSed base, escalation reserved for disagreement]. Codex bot loop landed three P2s across the PR, each fixed+replied: CliRunner signature probe; matched-but-skipped exit semantics (warnings before empty-check, exit 1 vs UsageError 2); skip-relevance gating so unrelated corrupt docs cannot masquerade a true no-match as an export failure. DeepSeek delta advisory (exit-1 path untested) already covered by the P2#2/#3 tests outside its review range; epilog gains the --include absolute-path note; get_root import hoisted (minimax). 1034 tests.                                                                                                                                                                                                                                                                         | Claude (Fable 5) |
