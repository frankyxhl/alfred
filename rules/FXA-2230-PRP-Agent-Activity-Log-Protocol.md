...
- Network filesystems (NFS / SMB / fuse-based remote mounts) for `rules/logs/`. `O_APPEND` and `flock(2)` semantics on these are implementation-defined and may silently no-op, breaking both the per-line atomicity guarantee and the archival serialization mutex. Local POSIX filesystems (ext4, xfs, btrfs, apfs, hfs+, ufs, zfs) are the v1 supported substrate. v2 may revisit if demand surfaces.
