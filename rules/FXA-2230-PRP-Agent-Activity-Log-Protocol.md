...
- Network filesystems (NFS / SMB / fuse-based remote mounts) for **either log root** — both the PRJ-layer `rules/logs/` and the USR-layer `~/.alfred/logs/` (which on enterprise / Kerberos / LDAP-managed Linux setups is frequently NFS-mounted via the user's `$HOME`). `O_APPEND` and `flock(2)` semantics on these substrates are implementation-defined and may silently no-op, breaking both the per-line atomicity guarantee and the archival serialization mutex regardless of which log root is active. Local POSIX filesystems (ext4, xfs, btrfs, apfs, hfs+, ufs, zfs) are the v1 supported substrate. The implementing CHG SHOULD detect the filesystem type of the resolved log directory at `af log` startup using the **OS-appropriate API** (Linux: `statfs(2)` `f_type` magic numbers — `NFS_SUPER_MAGIC = 0x6969`, `SMB_SUPER_MAGIC`, `CIFS_MAGIC_NUMBER`, `FUSE_SUPER_MAGIC` — accessed via `ctypes` to libc, **or** parse `/proc/self/mountinfo`; macOS / BSD: `statfs(2)` `f_fstypename` via `ctypes` to libc, or shell out to `stat -f "%T"`; **avoid `os.statvfs`** — it returns block/inode counts but does not expose filesystem type) and emit a one-time stderr warning on network-mount detection. A hard refusal is deferred to v2 to avoid breaking existing setups during rollout.
...
| 2026-05-02 | Address PR #80 R7 strict 4-reviewer review (3/3 received PASS — Codex worker exited prematurely without delivering, but PR was already merged so moot). 3 convergent advisory items collected post-merge as a clean follow-up patch (route B, no PR-block): (1) [L56, 3/3 satellite] `os.statvfs` is the wrong primitive for NFS detection — it returns block/inode counts but does NOT expose filesystem type. Implementers copy-pasting would silently never warn. Spec now mandates OS-appropriate API: Linux `statfs(2)` `f_type` magic numbers (NFS_SUPER_MAGIC=0x6969 etc.) via ctypes or parse `/proc/self/mountinfo`; macOS/BSD `statfs(2)` `f_fstypename` via ctypes or `stat -f '%T'`. Explicit 'avoid os.statvfs' anti-pattern note. Bug class: I had previously copied Gemini's R6 advisory verbatim into spec without verifying the API; Gemini caught it themselves in R7. (2) [L89, DeepSeek satellite] §Archival caveat still single-sourced 'rules/logs/' (PRJ) after L56 was extended to 'either log root'. Same self-contradiction class Codex bot has caught throughout — fixed L89 wording to 'either log root (PRJ rules/logs/ or USR ~/.alfred/logs/)' and added the `$HOME`-on-NFS rationale inline. (3) [L89, DeepSeek satellite] Stale internal reference '(line 55)' — Windows is L55, NFS is L56. Replaced with 'see the §Out of scope NFS bullet for the full rationale' to avoid future drift on line numbers. | Frank + Claude (PR #80 R7 satellite cleanup, post-merge) |
