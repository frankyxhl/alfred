...
- **Archival.** All closed days (any `YYYY-MM-DD.jsonl` or `YYYY-MM-DD.partN.jsonl` whose date is < today UTC) are folded into a **single `archive.zip` per log directory**, so the directory holds at most today's loose files plus `archive.zip`. The current day's file MUST stay loose because zip does not support append-atomic writes — emitting directly into a zip would force file-locking and defeat the 4 KiB / `O_APPEND` multi-process atomicity guarantee. Archival is performed by `af log` lazily (on the first invocation that detects ≥ 1 closed-day raw file) or on demand via `af log-archive`. **Procedure** — atomic for the archive replacement, eventually-consistent for raw-file cleanup:
  1. Build `archive.zip.tmp.<pid>.<rand6>` (PID + 6 random hex chars; per-process unique to avoid concurrent-write corruption when two `af log` startup archivers race) containing the union of the existing archive's entries plus the closed-day files. Filename collisions inside the zip are deduplicated by entry name — same `<date>.jsonl` from disk overwrites the existing zip entry; this is harmless because raw files are immutable once their day closes.
  2. `os.replace(archive.zip.tmp.<pid>.<rand6>, archive.zip)` — POSIX rename is atomic; this is the linearization point. Concurrent archivers race here: the loser's replace overwrites the winner's archive, but because every raw closed-day file is included by both, the resulting archive is content-identical regardless of who wins.
  ...
  4. **Tmpfile cleanup.** Before starting step 1, the archiver removes any stale `archive.zip.tmp.*` files older than 60 seconds (interrupted prior runs). Younger tmp files are left alone (probably another live archiver). The existing `archive.zip` is always authoritative.
...
- **Lazy archival on startup.** Before the first append in any invocation, `af log` scans the resolved log directory for any closed-day raw file (`YYYY-MM-DD.jsonl` or `YYYY-MM-DD.partN.jsonl` with date < today UTC). If found, it performs the atomic archival into `archive.zip` (see §Archival above) **before** writing today's record. This keeps the directory clean without requiring a daily cron.
  - **Failure handling.** If archival fails (filesystem error, corrupt existing archive, race lost), `af log` proceeds with today's append — observability MUST NOT block emit. Failures are logged to stderr **at most once per process** keyed by `(directory, error_class)`; subsequent identical failures within the same process are silently suppressed to avoid log spam from looping `af log` callers. The closed-day raw files remain on disk and the next `af log` invocation (in a fresh process) will retry — this is the self-healing path.
...
- **Per-directory atomicity**: follows the §Archival procedure (build `archive.zip.tmp.<pid>.<rand6>`; `os.replace`; unlink raw files with self-healing semantics on partial failure). No partial states are observable to readers — the archive replacement at step 2 is the linearization point.
...
- **Tmpfile cleanup**: stale `archive.zip.tmp.*` files older than 60 seconds are removed at start (interrupted prior runs); younger tmp files are left alone (probably another live archiver). The existing `archive.zip` is always authoritative.
