...
Patch the existing mermaid flowchart in `src/fx_alfred/rules/COR-1619-SOP-Orchestrator-vs-Worker-Dispatch.md` §Decision Tree by inserting two new gating nodes (`P` for parameter-gate and `TW` for two-worker-applicable gate) ahead of the existing `B{Generated file regen only?}` head of the tree. The existing chain from `B`, `MIX`, `H0`, `H`, `C`, `D`, `E`, `F`, `G` plus all leaf nodes (`O1/O2/O3/O4/W1/W2/W3/W4`) is preserved verbatim — diff against the pre-edit tree to confirm no existing node was deleted or reordered.
