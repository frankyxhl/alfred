...
| 2026-09-02 | R2/R3 review fixes (Codex PR #338, heads ec42562): slot guard searches the recursive USR scope (nested `USR-9000-*.md` conflicts; `logs/` excluded) (P1); UNC/extended-length roots (`\\server\share`, `\\?\C:\`) parse; atomic replace preserves existing registry mode / honors umask via shared `core.fsmode.resolve_write_mode`; `af projects` converts registry read failures to ClickException; `--prune` removes roots that became non-directories. 7 new tests (1601 total green)                                                          | alfred                                                                                                                                 |        |
| 2026-09-02 | R4 review fixes (Codex PR #338, head c2a8717): canonical rows backtick-quote the root cell so trailing whitespace in a path round trips exactly (legacy bare rows still parse); slot guard mirrors the scanner's full rules+logs exclusion predicate; `af projects` also converts invalid-UTF-8 read failures to ClickException; `af list`/`af status` re-scan when the trigger bootstraps USR-9000 (first-invocation visibility); CLAUDE.md inventory places register_cmd.py under commands/. 6 new tests (1607 total green)                      | alfred                                                                                                                                 |        |
| 2026-09-02 | R5/R6 batched review fixes (Codex PR #338, one commit per pfc convergence directive): ownership = unambiguous FXA-2330 template marker — table-shaped rows alone no longer mark a file as ours, so a custom doc at the registry filename is never replaced (P1); cross-layer preflight — a PRJ/other-layer USR-9000 doc blocks the registry write (trigger warns+skips, `af register` refuses; the registry itself excluded) (P1); symmetric cell encoding escapes backslash/pipe/backtick so `C:\dir\`, `/tmp/a                                   | b`, `/tmp/a\|b`, ``/tmp/a`b`` all round trip; dangling symlink at the slot is occupied, never replaced. 7 new tests (1614 total green) | alfred |
| 2026-09-02 | R7 batched review fixes: structured ownership marker `<!-- af:project-sop-registry v1 -->` replaces the FXA-2330-substring heuristic (legacy template line accepted as upgrade path — prose mentions never mark a doc as ours) (P1); the self-exemption covers only the canonical USR doc (usr source + canonical filename + top-level `~/.alfred`) — a PRJ doc carrying the canonical filename still blocks (P1); generated tables render column-aligned (canonical fmt style, pipes at identical positions) (P2). 4 new tests (1618 total green) | alfred                                                                                                                                 |        |
| 2026-09-02 | R8 review fixes: line-breaking characters (LF, CR, U+2028, U+2029) in roots encode as escape literals so rows never split (`C:\new` still round trips as backslash-n); recursive slot scan applies the scanner's is_file predicate — directories/FIFOs named USR-9000-*.md are not occupants; R5 history-row pipe escaped. 3 new tests (1621 total green)                                                                                                                                                                                          | alfred                                                                                                                                 |        |
| 2026-09-02 | R9 + CI repair: complete splitlines separator set encoded (VT/FF/FS/GS/RS/NEL added to the escape map; decode matches longest-literal-first so \x1c is never misread); non-regular occupants at the canonical slot (FIFO/dir/socket) rejected via stat BEFORE any open — read_text on a FIFO hung af projects/register and all four triggers (caught live by the R9 test); repo-wide ruff-format drift fixed (CI runs format --check on the whole tree, not just src/tests). 2 new tests (1623 total green)                                        | alfred                                                                                                                                 |        |
| 2026-09-02 | R10 (round cap): ACID-only lookups resolve against the pre-bootstrap snapshot — an unambiguous `af read 9000` (project carries FXA-9000) stays unambiguous instead of turning error once USR-9000 exists; only a not-found retry consults the rescan (the R1 bootstrap path is preserved). Terminal-display escaping for pathological control-char roots scope-lined per pfc ruling (tracked follow-up, not a merge blocker). 1 new test (1624 total green)                                                                                        | alfred                                                                                                                                 |        |
