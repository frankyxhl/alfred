...
- States the "when most useful" heuristic (5+ rounds of GitHub App bot iteration where remaining items are tightening edge cases vs catching defects), the explicit non-substitution caveat (not a way to suppress real findings), and a "when NOT useful" note (skip if the installed bot already produces zero off-scope findings **AND** per-round finding volume is already acceptably low; or for small / uncertain-scope PRs with no prior signal that the installed bot over-flags). The compound AND condition matters: the alfred evidence base itself comes from a bot already at zero off-scope pre-hints, where the value the technique added was per-round volume reduction (1.5 → 1.0); a heuristic of "skip if zero off-scope" alone would have wrongly excluded the very repo class that produced the evidence. The "small / uncertain scope" rule has its own caveat — PRs known up-front to be long-running (large CHG/SOP edits, multi-file refactors) should add hints from R1, matching the documented usage pattern of alfred PR #119 and #120.
...
| 2026-05-09 | R4: codex bot R3 P2 (PR #122) — cross-doc drift after R3's COR-1612 skip-rule narrowing. The implementation in COR-1612 was updated (small/uncertain-scope rule + caveat for known-long PRs) but this CHG's §What item 1 last bullet still cited the old "fewer than 3 review rounds" wording, leaving CHG and implemented SOP contradictory. Updated §What item 1 last bullet to match the implemented rule + caveat. CHG-2279 R4. | Claude Opus 4.7 |
