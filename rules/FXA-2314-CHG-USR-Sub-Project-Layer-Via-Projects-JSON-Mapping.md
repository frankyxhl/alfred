...
10. **Write path is symmetric with the read path** (added per code-review P1) — project-layer *writes* also follow the mapping. `af create --layer project` and `af index` route through a shared `core.projects.project_layer_dir(root)` helper that returns `~/.alfred/<NAME>/` for a mapped root (else `<root>/rules/`). Without this, those writers would create files in a mapped third-party repo's `rules/` (the pollution we are avoiding) and the files would be invisible to the redirected read path. Unmapped roots write to `<root>/rules/` exactly as before.

11. **`af validate` error path stays mapping-aware** (added per code-review P2) — `scan_documents` gains `validate_layers: bool = True`; when `False` it returns the (mapping-applied) corpus without raising on layer violations. `validate`'s fallback now calls `scan_documents(root, validate_layers=False)` instead of a parallel re-scan, so the redirect/exclusion applies even on the diagnostic path. The old duplicate `validate_cmd._scan_all_layers` is deleted — one scan implementation that cannot drift.
...
- **Systems affected:**
  - `src/fx_alfred/core/scanner.py` — `scan_documents` (unconditional mapping-wins recursive PRJ redirect + global USR exclusion). Reuses existing `_scan_path_dir(..., recursive=True)` and its `logs/` skip for the redirected layer. Gains `validate_layers: bool = True` (§What-11) so the corpus can be returned without raising on the validate error path.
  ...
  - **New** `src/fx_alfred/core/projects.py` — load/parse `~/.alfred/projects.json` once with graceful fallback; expose `resolve_subproject(root) -> str | None` (consumed by scanner exclusion + `context` root discovery) and `project_layer_dir(root) -> Path` (the write-side equivalent, §What-10), the single sources of truth.
  - `src/fx_alfred/commands/create_cmd.py` + `index_cmd.py` — project-layer write base routed through `project_layer_dir(root)` (§What-10, code-review P1).
  - `src/fx_alfred/commands/validate_cmd.py` — error-path fallback uses `scan_documents(root, validate_layers=False)`; parallel `_scan_all_layers` deleted (§What-11, code-review P2).
  - `tests/` — new coverage in `test_scanner.py`, `test_guide_cmd.py`, `test_list_cmd.py`, `test_create_cmd.py`, `test_index_cmd.py`, `test_validate_cmd.py`, and a new `test_projects.py` for the loader.
  - Docs — `README.md` / `CLAUDE.md` three-layer-model section gains a `projects.json` note (pairs with the existing `af create --subdir`).
  - **Read-only scan-routing commands unchanged** — `guide`, `list`, `read`, `status`, `search`, `where`, `export`, `plan` inherit the redirect purely via `scan_documents`/`scan_or_fail` (no per-command edits). The write/diagnostic commands `create`, `index`, `validate` needed the targeted edits above (§What-10/11) because they own write targets / a non-raising scan path.
...
| 2026-06-28 | PR-bot (Codex) P1 fix — write path made symmetric with read (§What-10): `af create --layer project` + `af index` route project-layer writes through new `core.projects.project_layer_dir(root)`, so a mapped third-party repo's `rules/` is never polluted and writes are visible to the redirect. Tests added (create/index mapped-context). make check green.                                                                                                                                                                                                                                             | Claude Code |
| 2026-06-28 | PR-bot (Codex) P2 fix — `af validate` error path made mapping-aware (§What-11): added `scan_documents(validate_layers=False)` and deleted the parallel `validate_cmd._scan_all_layers`; one non-drifting scan implementation. RED test pins the mapped error-path classification. make check green (1262 tests).                                                                                                                                                                                                                                                                                            | Claude Code |
| 2026-06-28 | Doc-tooling fix — regenerated `FXA-0000` index with the repo's `.venv/bin/af` (the pipx `af` had rewritten it into an outdated 3-column format, ~447-line churn); index diff is now just the new 2314 row + dates.                                                                                                                                                                                                                                                                                                                                                                                          | Claude Code |
