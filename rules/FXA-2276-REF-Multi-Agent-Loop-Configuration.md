...
| Phase | Today | Notes |
|---|---|---|
| 1 — Auto-pick | ⚠ conditional | **Default sessions** (no `follow FXA-2276` invocation): user-initiated; explicit ACID picks (`do FXA-NNNN`) are bypassed per COR-1618 §Normative Bypass Clause. **Looping mode** (under `follow FXA-2276` or `follow FXA-2276 once` — see §Invocation): every pick applies full COR-1618 `verify_consent_eligibility`, including the first — `follow FXA-2276` directs the orchestrator to start the loop but does not name an issue, so the agent's pick is not user-directed under COR-1618's strict definition. **User-directed-pick mode** (`follow FXA-2276 for #N`): the named issue is bypass-eligible per §Normative Bypass Clause and runs phases 2–11 regardless of its rocket-gate state. |
