...
**Trigger**: a review finding (bot, panel, or human) whose scope condition varies along one or more **enumerable dimensions** — any axis whose values form a small closed set. Canonical axes: actor × timing window (races: processes, threads, reader vs. writer), ID origin × outcome (explicit / auto / always-injected / always∩user-matched × valid / skipped), output mode × input class (text vs `--json` as the base mode, with `--todo`/`--graph` as ADDITIVE flags that combine with either — enumerate combinations, not a flat list — × well-formed / malformed / missing), platform × capability (POSIX / non-POSIX × probe available / absent). If the finding's wording names a category ("when X is also Y", "only for Z-mode"), the dimension is enumerable and this rule applies. The fix round MUST:
