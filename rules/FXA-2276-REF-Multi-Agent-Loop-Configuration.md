...
**Alfred deviation from COR-1617 §Phase 1**: COR-1617 §Phase 1's User-driven trigger row lists `"pick next issue"` and `"auto-pick"` as bypass-eligible — but those phrases name no target issue, while COR-1618 §Normative Bypass Clause restricts bypass to chat text that **explicitly names the target issue**. Alfred resolves the PKG-internal tension in favor of COR-1618's strict reading: `follow FXA-2276` and `follow FXA-2276 once` (no target ACID) are treated as autonomous picks and run full COR-1618 + COR-1506 on every pick. This is an intentional, documented deviation; PKG reconciliation between COR-1617 §Phase 1's phrase list and COR-1618's strict definition is tracked separately.
