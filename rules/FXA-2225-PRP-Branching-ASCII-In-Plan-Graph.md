...
3. **Extend step-ID parsing** — accepts `\d+[a-z]?` for sub-step support. All-integer SOPs (every existing SOP in the corpus) keep working unchanged. Storage shifts from `frozenset[int]` to `frozenset[str]` with `str(int)` for legacy entries. The `--json` output's step-ID type changes from `int` to `str` — documented as a v1.8.0 breaking note for any downstream JSON consumer.
   ...
   **Migration Impact Table** — every consumer of `step_indices` and integer step IDs that needs auditing. Pre-verified by Codex Round 1 review:
   ...
   | `commands/plan_cmd.py:191/278/283/344` `dict[int, LoopSignature]` keys | int keys | str keys | Plan-builder rekeys; affects `--todo` and `--graph` output keying | Plan-builder unit test with sub-step IDs |
   | `core/ascii_graph.py:134/360/397` int-indexed step iteration | int | str (with `str.isdigit()` shortcut for legacy hot paths) | Touch every loop bound that assumes `range(1, n+1)` | Render fixture: SOP with `1, 2, 3a, 3b, 4` |
   | `core/steps.py:45/59` step-line regex `(\d+)\.` | int (cast on line 47) | str | Drop the `int(...)` cast at line 47; `gate` detection unchanged | Existing gate tests run; add sub-step gate test |
...
4. **Renderer extension** — when a phase contains a step with a matching `Workflow branches` entry, the layout draws:
   ...
   - Edge label (from `to[i].label`) printed above each branch arrow, centered under the connector tee, truncated to 12 cells with `…` per `wcwidth`
   ...
   **Geometry algorithm sketch** (avoids "redesign at coding time" — Gemini Round 1 fix #3): given N siblings rendered at column offsets `c_1..c_N` (column `c_i` is the center of sibling box `i`):
   ...
   - Edge labels print on the row between the connector and the sibling box top — centered between adjacent connector tees, max-width = `(c_{i+1} - c_i) - 2`.
...
7. **`af validate` cross-checks** — `from` references existing integer step; each `to.id` references an existing sub-step; sub-step IDs are well-formed (`\d+[a-z]`); leading integer of every `to.id` equals `from + 1`; no orphan sub-steps (every `Xa`/`Xb` mentioned in `## Steps` must appear in some `Workflow branches.to` list); siblings are contiguous in `## Steps` (no integer steps interleaved); convergence target (next-sequential integer step after branch group) exists OR all siblings have explicit terminal markers (Cut 3 `joins:` field — Mid raises an error if convergence is implied but next integer is missing); label cell-width via `wcwidth.wcswidth()` ≤ 12.
...
If this defense doesn't satisfy Round 2 reviewers, fallback is to drop auto-convergence from Mid (siblings have dangling tails) and ship `joins:` as a Cut 3 PRP. Cost of that fallback: every common-case branchy SOP requires `joins: 4` boilerplate authoring. Document this fallback explicitly so reviewers can choose.
...
| Width math fails on terminals < 80 chars (4 siblings × ~18-cell boxes ≈ 80 cells minimum) | Document minimum-width assumption (80 cells); fall back to single-column rendering (each sibling on its own row, indented under parent) when terminal width < 80. Detect via `shutil.get_terminal_size()`; pipe-output assumes 80. |
...
| Auto-convergence "structured-position inference" rejected by reviewers as inconsistent with OQ-1 anti-prose-inference principle (raised by Gemini Round 1) | Defense argued in OQ-4 above; fallback ready (drop auto-convergence from Mid; ship `joins:` as Cut 3 first). Decision deferred to Round 2 reviewer consensus. |
