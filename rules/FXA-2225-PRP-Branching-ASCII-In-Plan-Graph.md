...
**Status:** Approved
**Reviewed by:** Codex 9.2, Gemini 9.30 (Round 3)
...
3. **Extend step-ID parsing** — accepts `\d+[a-z]?` for sub-step support. All-integer SOPs (every existing SOP in the corpus) keep working unchanged. Storage shifts from `frozenset[int]` to `frozenset[str]` with `str(int)` for legacy entries. The `--json` output's step-ID type changes from `int` to `str` — documented as a v1.8.0 breaking note for any downstream JSON consumer.
   ...
   **Migration Impact Table** — every consumer of `step_indices` and integer step IDs that needs auditing. Pre-verified by Codex Round 1 review:
   ...
   | `core/ascii_graph.py` — int-indexed step iteration. **Signature lines: 134/360/397; loop bodies / iteration sites: 173, 219, 265.** Every `step_idx` and `step_indices` reference in this file needs auditing — count via the self-audit grep below (currently 6 `step_idx` + 12 `step_indices` lines, union 15; specific count will drift as code evolves so don't memorize the number). | int | str (with `str.isdigit()` shortcut where legacy hot path matters) | Touch every loop bound that assumes `range(1, n+1)`; review every `for i, step_idx in enumerate(step_indices)` (e.g. line 173) | Render fixture: SOP with `1, 2, 3a, 3b, 4` exercising all hits |
...
| Capability over-fit | mermaid-ascii solves the *general* DAG-layout problem (edge crossings, mixed graph types). Mid scope is much narrower: known parent → labeled siblings → auto-detected next-sequential convergence. Realistic Mid implementation cost (per Scope §3-§7 + Risks reconciled Round 3): **~150–250 LOC** broken down by component for implementation-PR auditability — *parser widening* ~30 (step-ID regex + cast removal in `core/steps.py`); *branch geometry* ~80 (column offsets, tee placement, label centering, convergence join math); *validator rules* ~40 (sub-step contiguity, `from`/`to` cross-checks, sibling-leading-integer constraint); *flat/nested shared-primitive factoring* ~80–100 (extract branch geometry from `dag_graph.py` so `flat` reuses it without phase-box wrapping); *wcwidth truncation* ~20 (12-cell label cap with `…`). Well below mermaid-ascii's full layout engine but explicitly *not* the "30-40 lines" framing used in Round 1/2 (Round 2 reviewers correctly flagged that estimate as undercooked). |
...
**Two regex roles — not the same pattern:**
- **Sub-step ID format** (the *new* primitive a sub-step must match): `\d+[a-z]` — mandatory suffix letter. This is what `Workflow branches.to[i].id` must satisfy.
- **Parser/storage format** (what `core/steps.py` and `core/workflow.py` accept when scanning `## Steps` lines or step-ID strings generally): `\d+[a-z]?` — optional suffix. This admits both legacy all-integer SOPs (`1`, `2`, `3`) AND new sub-stepped ones (`3a`, `3b`). The literal OQ-2 resolution `\d+[a-z]` would reject every existing SOP if applied to the parser; it is the *sub-step* format only. The parser uses the optional-suffix form.
...
| Multi-model review divergence between Codex and Gemini on auto-convergence | Mitigated by Round 2 outcome — Gemini accepted the defense; Codex did not challenge. Auto-convergence stays in Mid; the previously-considered fallback (drop auto-convergence; defer to Cut 3) is no longer in play. Risk row retained for historical traceability. |

---

## Approval

- [x] Approved by: Frank Xu on 2026-04-27 (per multi-model review COR-1602: Codex 9.2 PASS, Gemini 9.30 PASS — both ≥ 9.0 threshold)
...
| 2026-04-27 | Round 3 review PASS (Codex 9.2, Gemini 9.30 — both ≥ 9.0 threshold). Status flipped Draft → Approved. Final advisory cleanup applied: (a) `ascii_graph.py` "15 hits" claim reworded to use the self-audit grep directly (Codex flagged the cited grep returns 6, not 15 — actual union of `step_idx` + `step_indices` IS 15; clarified); (b) regex disambiguation `\d+[a-z]` (sub-step ID format, mandatory suffix) vs `\d+[a-z]?` (parser/storage format, optional suffix) added to OQ-2 — without this, literal application of OQ-2's `\d+[a-z]` would reject every existing all-integer SOP; (c) stale "OQ-4 fallback re-dispatch path" risk row reworded — fallback no longer in play; (d) ~150-250 LOC estimate decomposed by component (parser ~30, geometry ~80, validator ~40, factoring ~80-100, wcwidth ~20) per Gemini Round 3 advisory for implementation-PR auditability. | Frank + Claude Code |
