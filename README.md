...
- **NEW in v1.29.0** — **`af create` numbers sequentially by default**: omit both `--acid` and `--area` and the next ACID for the prefix is allocated for you (highest existing + 1, the 0000 index counts, never a gap-fill). Allocation scope is layer-aware (a user-layer write numbers against the whole registered `~/.alfred/<NAME>` unit — or against every unit for global writes), serialized by a single per-user create lock (`~/.alfred/.af-create.lock`, flock/msvcrt, `AF_CREATE_LOCK_TIMEOUT`, dry runs lock-free), and destination-guarded: scanner-invisible paths (`logs/` trees judged relative to the scan root), symlink escapes out of the scanned tree, and the reserved lock file name are all refused ([#330](https://github.com/frankyxhl/alfred/pull/330)).
- **NEW in v1.28.0** — [COR-1615 (GitHub App PR Review Bot Loop)](src/fx_alfred/rules/COR-1615-SOP-GitHub-App-PR-Review-Bot-Loop.md) gains an **Agent Execution ladder**: the PR review-bot loop is now executable by turn-based agents, not only human operators. Rung A binds COR-1620 wakeup primitives, rung B is a bounded blocking wait script (gh + awk only; `SINCE`/`BOT_USER`/head-recheck filters, exit-code routing incl. gh-failure exit 3), rung C mandates a resumable handoff note — with a binding rule that ending the task silently while a review request is pending is a SOP violation, and a CI-settling generalization via bounded `gh pr checks` polls. A second machine-readable back-edge (`poll-wait`, 8→6) makes the waiting loop parser-visible. The skills/alfred contract (all carriers) now self-activates the loop on push/PR-create/ready-for-review, self-heals stale installs, and reduces cleanly in no-bot repos. Battle-tested on its own delivery PR: 15 live codex review rounds + a three-model panel shaped the final filters.
