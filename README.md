...
- **NEW in v1.25.1** — Patch release: two silent data-loss paths in the activity-log archive are closed (concurrent appends can no longer be lost mid-archive: #263/#289; re-archiving a reappeared loose file now merges same-named `archive.zip` members instead of destroying archived rows, with matching shadow-union read semantics: #264/#290), `af create` refuses empty-slug titles that produced scanner-invisible orphan files (#265/#291), and fence handling is unified across write/scan paths (#262/#288).
