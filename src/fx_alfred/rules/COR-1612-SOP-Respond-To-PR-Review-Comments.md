...
- The PR has fewer than 3 review rounds — there isn't enough data to know whether the bot is over-flagging.
...
The technique is a **directive about which findings to defer to a follow-up batch**, not a way to suppress findings. Bots that honor scope hints still flag in-scope items at the same rate; the change is in what they spend rounds on, not whether they catch real defects. If a PR uses scope hints and the bot stops flagging known-broken behavior, the technique is being mis-applied (or the bot isn't honoring hints — verify empirically).
