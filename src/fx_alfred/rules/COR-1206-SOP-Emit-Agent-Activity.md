...
This document defines the **emit protocol**. The **format** lives in COR-1205. The **CLI surfaces** are `af log` (writer), `af log-validate` (schema checker), and `af log-archive` (explicit archival). FXA-2307 implements the CLI surfaces and first-party `af guide` / `af plan` command telemetry; native per-agent hook recipes remain the integration contract below.
