...
| 2 | At least one `<consent-signal>` reaction from any member of `<repo-trusted-reactor-list>` exists on the issue body | `gh api repos/<repo>/issues/<N>/reactions --paginate \| jq -rs --argjson trustees '<repo-trusted-reactor-list-as-json-array>' --arg signal '<consent-signal>' 'flatten \| map(select(($trustees \| index(.user.login)) and .content == $signal)) \| first'` (slurp pages with `jq -rs`; without slurp, `--jq` runs per-page and emits `null\nnull\n...` for unsignaled issues, letting them slip through. Use `index` for array membership — jq's `IN(...)` is a stream-membership test, not array-membership, so a list-typed parameter cannot be passed via `IN`.) |
