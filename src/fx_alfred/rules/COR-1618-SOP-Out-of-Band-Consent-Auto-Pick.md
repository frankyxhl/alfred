...
| 3 | No invalidating timeline events at-or-after `<signal-ts>` (the `created_at` captured by check 2) | `gh api repos/<repo>/issues/<N>/timeline --paginate \| jq -ers --arg signal_ts '<signal-ts>' 'flatten \| map(select((.event \| IN("edited","renamed","closed","reopened","transferred","unlocked")) and (.created_at >= $signal_ts))) \| length == 0'` (use `>=` not `>` — same-second mutations fail closed. `<signal-ts>` MUST be passed via `--arg signal_ts`; without that, the recipe compiles with `$signal_ts is not defined` and every re-verification fails closed before any git op. **`length == 0` + `-e` is load-bearing**: without the boolean-and-`-e` shape, the recipe emits the matched events and exits 0 whether the array is empty or non-empty — check 4 then treats invalidators-present as "no error" (fail-OPEN). With `-e` + the boolean test, jq exits 0 only when no invalidators exist; the presence of any invalidator makes jq emit `false` and exit 1.) |
