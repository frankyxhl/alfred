...
| 2 | At least one `<consent-signal>` reaction from any member of `<repo-trusted-reactor-list>` exists on the issue body. Capture the matched reaction's `created_at` as `<signal-ts>` for check 3. | `gh api repos/<repo>/issues/<N>/reactions --paginate \| jq -rs --argjson trustees '<repo-trusted-reactor-list-as-json-array>' --arg signal '<consent-signal>' 'flatten \| map(select(.user.login as $u \| ($trustees \| index($u)) and .content == $signal)) \| first'` (slurp pages with `jq -rs`; without slurp, `--jq` runs per-page and emits `null\nnull\n...` for unsignaled issues, letting them slip through. **`.user.login as $u` first** — without the `as`-binding, `$trustees \| index(.user.login)` switches input to the trustees array and `.user.login` then evaluates against that array, raising "Cannot index array with string user". `index` is the right primitive — jq's `IN(...)` is a stream-membership test, not array-membership, so a list-typed parameter cannot be passed via `IN`.) |
| 3 | No invalidating timeline events at-or-after `<signal-ts>` (the `created_at` captured by check 2) | `gh api repos/<repo>/issues/<N>/timeline --paginate \| jq -rs --arg signal_ts '<signal-ts>' 'flatten \| map(select((.event \| IN("edited","renamed","closed","reopened","transferred","unlocked")) and (.created_at >= $signal_ts)))'` (use `>=` not `>` — same-second mutations fail closed. `<signal-ts>` MUST be passed via `--arg signal_ts`; without that, the recipe compiles with `$signal_ts is not defined` and every re-verification fails closed before any git op.) |
