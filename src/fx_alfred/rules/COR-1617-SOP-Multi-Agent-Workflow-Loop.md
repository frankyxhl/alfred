...
=======
| 2026-05-09 | R2 (PR #119): codex bot R1 P2 — §Phase 7 `gh pr create --head` form was hardcoded to `<gh-write-identity>:<branch>`, which works for fork-PR but breaks for single-remote topology where the branch lives on `<repo-owner>`'s repo, not `<gh-write-identity>`'s fork. Snippet now documents both forms with topology-conditional comment; `--head <head-spec>` placeholder lets the adopter substitute per their COR-1622 instantiation. Routing-table row also updated to point at the §Phase 7 snippet rather than hardcode the fork form. | Claude Opus 4.7 |
>>>>>>> Stashed changes
