...
3. **Orchestrator commits** the test files with prefix `test:` per COR-1500 §Commit Strategy Option A. Before proceeding to step 4, the orchestrator performs a **test-quality gate** satisfying COR-1500 §AI-Assisted TDD Protocol Mandatory Rule #4 (test-quality review before GREEN). The two-worker split structurally addresses Rule #4's primary failure mode (tests mirroring implementation — the test-writer had no implementation to mirror, by construction), but residual failure modes are NOT addressed by the structure alone and require explicit inspection of the test diff:
   ...
   On detection of any residual failure mode: reject the test-writer's work and re-dispatch with the specific finding as feedback (counts against step 2's 2-retry cap). Stronger projects MAY substitute a plan-review-panel inspection of the test diff for the grep-based gate.
