...
8. **Phase 8 (Iterate) re-dispatch routing.** When COR-1617 §Phase 8 surfaces a new finding requiring code (bot review comment, CI failure, panel finding), the orchestrator decides between four mutually exclusive cases:
   - **(a) New test+implementation pair needed** (e.g., reviewer asks "add a test for the empty-input edge case and handle it") → re-enter this contract from step 1 with the new scope, **bypassing step 2's RED-must-fail gate** for this iteration (step 2 unconditionally rejects passing tests as if they failed to constrain the un-implemented code; in the additive-coverage sub-case the test legitimately passes against current HEAD, so step 2's reject path would make the additive-coverage branch unreachable). After the test-writer dispatch returns, the orchestrator skips step 2 and runs the test-quality gate from step 3 directly on the un-committed worktree (vacuity / trivial RED / zero-assertion checks apply unchanged). On gate pass: orchestrator commits with `test:` prefix per step 3. Orchestrator THEN runs `<test-runner>` locally on the just-committed test; **if the test fails** (the implementation does not yet cover the requested behaviour, expected RED), continue to step 4 to dispatch implementer (which produces a `feat:`/`fix:` commit per step 6); **if the test passes** (the existing implementation already covers the requested behaviour, additive coverage), skip implementer dispatch entirely — the test-writer's step-3 `test:` commit is the final commit for this iteration, no implementation commit is produced, and the iteration is done. This local-run gate (after commit, not before) exists because additive-coverage requests are common in PR review and forcing a redundant implementer round-trip burns latency and risks hallucinated changes.
