...
1. **Dispatch test-writer worker** (`<test-writer-worker-agent>`) with the COR-1619 §Worker Dispatch Contract items, plus:
   ...
   - *Enforcement note:* the push/commit constraint is orchestrator-trust-based; the dispatch backend may not enforce read-only mounts or commit-blocking flags. Orchestrator detects violations post-dispatch by comparing the worker's reported file list against `git status --porcelain` after the dispatch returns (the porcelain XY format covers both index state and worktree state, and since both workers commit through the orchestrator's identity, `git log --author=` cannot differentiate them).

       **Cleanup contract** (on detection of a violation — worker modified production source files outside the reported list, OR reported a production-source edit, OR wrote files outside the test paths): the orchestrator MUST reset every off-test-path entry to its HEAD state — both index and worktree match HEAD, untracked additions are deleted, and rename entries are decomposed into restore-source + clean-target. Cleanup is whether-or-not the path appeared in the worker's reported list (a reported production-source edit is itself a violation of the RED-phase output constraint). The contract is normative; the implementation guidance below is non-exhaustive.

       **Implementation guidance** for each `git status --porcelain` entry whose path is off-test:
       - **Tracked modifications / staged adds** (porcelain first char ∈ `M A D`, second char in ` M D`, etc.): `git restore --staged --worktree -- <path>` resets both index and worktree from HEAD. For staged-add entries (`A  path`) where HEAD doesn't contain the file, `--staged` un-stages it (leaving it as `??`); combine with `git clean -f -- <path>` to delete the untracked residual.
       - **Untracked additions** (`?? path`): `git clean -f -- <path>` (a bare `git restore` is a no-op on these).
       - **Renames** (`R  oldpath -> newpath`): the rename target is staged (`A  newpath` in the index after one decomposes the porcelain) — `git clean -f` is a no-op on staged adds, so the target MUST be un-staged before it can be cleaned. Use the three-step sequence: `git reset HEAD -- <oldpath> <newpath>` (unstages both paths — oldpath returns to worktree-deleted, newpath returns to untracked-added), then `git checkout HEAD -- <oldpath>` (restores the original path's worktree content), then `git clean -f -- <newpath>` (deletes the now-untracked newpath). A single `git restore --staged --worktree -- <path>` cannot handle a rename entry — the porcelain reports both paths in one line and a single operation either errors on the literal `old -> new` text or leaves the staged target behind.
       - *Important:* there is no single one-shot batch command that handles every porcelain state across a mixed path set — `git checkout HEAD -- <paths>` aborts with `pathspec ... did not match any file(s) known to git` whenever the path set contains any entry not in HEAD (untracked additions, staged adds), leaving the entire batch unprocessed. Operators MUST classify each off-test path by its `git status --porcelain` XY prefix and apply the per-class command above on a per-path (or per-state-group) basis; do not batch heterogeneous paths through a single `git checkout` or `git restore --source=HEAD`.

       Orchestrator then surfaces the violation in the dispatch log + CHG `## Implementation Order`, and re-dispatches the test-writer with the violation as explicit feedback. Repeated violations (≥2 from the same worker on the same task) escalate per step 2's retry cap.
...
3. Before committing, the orchestrator performs a **test-quality gate** on the test-writer's worktree changes, satisfying COR-1500 §AI-Assisted TDD Protocol Mandatory Rule #4 (test-quality review before GREEN). The gate runs on the un-committed worktree so that rejected tests never enter the branch history (running it post-commit would leave a polluted `test:` commit on the branch even after re-dispatch — the next re-dispatch would layer on top of the bad commit, not replace it). The two-worker split structurally addresses Rule #4's primary failure mode (tests mirroring implementation — the test-writer had no implementation to mirror, by construction), but residual failure modes are NOT addressed by the structure alone and require explicit inspection:
   - **Vacuity**: `grep -nE "^\s*assert (True|False|0|1|None|not None)\s*(#.*)?$" <test-writer-paths>` SHOULD return empty (those patterns flag trivially-true or trivially-false assertions that pass step 2's RED gate without constraining real behavior).
   - **Trivial RED**: test functions whose body consists solely of `pass`, `raise NotImplementedError`, `pytest.fail(...)`, or unconditional `pytest.xfail(...)` — these fail at step 2's RED gate but the failure is structural, not behavioural.
   - **Zero-assertion bodies**: `grep -L "assert " <test-writer-paths>` returns each path containing no `assert` keywords.
   On detection of any residual failure mode: reject the test-writer's work and re-dispatch with the specific finding as feedback (counts against step 2's 2-retry cap). Before re-dispatch, the orchestrator MUST clean the worktree to remove BOTH (a) any off-test-path entries per step 1's Cleanup contract AND (b) the rejected test files themselves at `<test-writer-paths>` (which live INSIDE the allowed test paths and so are not covered by step 1's off-test-path scope) — for tracked entries `git restore --source=HEAD --staged --worktree -- <test-writer-paths>`, for newly-created test files at paths not in HEAD use `git reset HEAD -- <new-test-paths>` then `git clean -f -- <new-test-paths>`. No `test:` commit happens for the rejected attempt. Stronger projects MAY substitute a plan-review-panel inspection of the test diff for the grep-based gate.

   On gate pass: **orchestrator commits** the test files with prefix `test:` per COR-1500 §Commit Strategy Option A.
...
8. **Phase 8 (Iterate) re-dispatch routing.** When COR-1617 §Phase 8 surfaces a new finding requiring code (bot review comment, CI failure, panel finding), the orchestrator decides between four mutually exclusive cases:
   - **(a) New test+implementation pair needed** (e.g., reviewer asks "add a test for the empty-input edge case and handle it") → re-enter this contract from step 1 with the new scope. **Pre-dispatch classification is required**: the orchestrator MUST classify the reviewer's request as either (a.i) `new-behavior-and-test` (default — the reviewer expects a production change as well as the test, e.g. "handle it"/"fix it"/"add the case") OR (a.ii) `coverage-only` (rare — the reviewer explicitly states the implementation already handles the case and only asks for additional regression coverage, e.g. "add a test asserting current behavior", "add coverage for this scenario"). The classification is recorded in the iteration's CHG `## Implementation Order` row before dispatch.
     - **For `new-behavior-and-test` (a.i)**: step 2's RED-must-fail gate applies unchanged. A passing test is REJECTED (the test-writer didn't author a behavioural constraint, or accidentally wrote a semantically wrong test that passes against current code). Re-dispatch with the rejection feedback (counts against step 2's 2-retry cap). On step 2 pass + step 3 quality gate pass → step 4 dispatch implementer → step 6 `feat:`/`fix:` commit. The standard two-worker flow.
     - **For `coverage-only` (a.ii)**: step 2's RED gate is bypassed (the test is expected to pass). After test-writer dispatch returns, orchestrator runs the test-quality gate from step 3 directly on the un-committed worktree (vacuity / trivial RED / zero-assertion checks still apply — coverage tests must still be substantive). On gate pass: `test:` commit. Orchestrator THEN runs `<test-runner>` to confirm the test passes against current HEAD; **if the test passes** (confirming additive coverage), done — the `test:` commit is the final commit for this iteration. **If the test FAILS** despite the `coverage-only` classification (the reviewer's premise was wrong — the implementation actually doesn't cover the case), escalate: surface the misclassification to the reviewer/user, do NOT silently dispatch implementer, and do NOT commit the test (`git restore` cleanup per step 3 rejection path). The misclassification likely means the reviewer intended (a.i) but used (a.ii) language; require explicit confirmation before continuing.
     
     The classification cannot be skipped: a blanket bypass of step 2 for all case-(a) iterations would let an accidentally-passing wrongly-written test (in an `new-behavior-and-test` request) commit and skip the implementer, marking the reviewer's edge-case request done with no production change.
...
| 2026-05-17 | R7 (PR #177 codex bot P2 on R6 head): **FXA-2289** step 1 enforcement note — comprehensive rewrite from recipe-style commands to **contract + implementation guidance** to address codex's escalating series of git-edge-case findings (R2/R3/R5/R7 all targeted this section incrementally). The new wording: (a) declares the normative **Cleanup contract** — every off-test-path entry must be reset to HEAD state, untracked additions deleted, renames decomposed into restore-source + clean-target — independent of specific commands; (b) provides non-exhaustive **Implementation guidance** covering tracked entries (M/A/D), untracked (`??`), renames (`R  old -> new`), and a catch-all (`git reset` + `git checkout` + `git clean`). The R7 finding triggering this rewrite was the rename case (`R  src/a.py -> src/b.py`) — a single `git restore --staged --worktree -- <path>` cannot handle a porcelain rename entry because the path string is "old -> new" not a single pathspec. Same PRP-1507 deviation note as R2-R6. | Claude Opus 4.7 |
| 2026-05-17 | R8 (PR #177 codex bot P2 on R7 head): **FXA-2289** step 1 rename guidance — R7's first-option rename recipe (`git restore --staged --worktree --source=HEAD -- <oldpath>` AND `git clean -f -- <newpath>`) leaves the staged-add rename target intact, because `git clean -f` is a no-op on staged adds (`A  newpath`). The "or alternate" sentence had the correct unstage-then-rm sequence, but operators following the first option literally would still ship the staged rename target with the `test:` commit. Fix: rename guidance now mandates the three-step sequence `git reset HEAD -- <oldpath> <newpath>; git checkout HEAD -- <oldpath>; git clean -f -- <newpath>` (un-stages both paths, restores oldpath worktree, deletes the now-untracked newpath); the prior "either-or" wording is gone. Same PRP-1507 deviation note. | Claude Opus 4.7 |
| 2026-05-17 | R9 (PR #177 codex bot P2 on R8 head): **FXA-2289** step 1 catch-all removed — R7's catch-all (`git reset HEAD -- <paths>` + `git checkout HEAD -- <paths>` + `git clean -f -- <paths>`) was itself broken: `git checkout HEAD -- <paths>` aborts with `pathspec ... did not match any file(s) known to git` when any path is not in HEAD (untracked additions, staged adds), leaving the entire batch unprocessed. Fix: replaced the broken catch-all with an explicit warning that there is no single one-shot batch command for heterogeneous path sets; operators MUST classify each path by its porcelain prefix and apply the per-class command. Same PRP-1507 deviation note. | Claude Opus 4.7 |
| 2026-05-17 | R10 (PR #177 codex bot P2 on R9 head, soft cap evaluation): **FXA-2289** step 3 extended with a **test-quality gate** satisfying COR-1500 §AI-Assisted TDD Protocol Mandatory Rule #4. R10's finding shifted class from git-edge-cases (R2-R9) to a semantic SOP gap: the two-worker dispatch skipped Mandatory Rule #4 (test-quality review before GREEN) entirely. Fix: step 3 now includes pre-step-4 inspection covering (a) vacuity (`assert True/False/0/1/None`), (b) trivial RED (`pass`/`NotImplementedError`/`pytest.fail`/`xfail`), (c) zero-assertion bodies (`grep -L "assert "`). The two-worker split structurally addresses Rule #4's primary failure mode (impl mirroring — test-writer has no impl), but residual modes (vacuity, trivial-RED) still apply. Detection rejects + counts against step 2's 2-retry cap. Stronger projects MAY substitute panel-review of test diff. R-count cap evaluation per FXA-2276: R10 finding is P2, so Case B (P0/P1/P2 open) self-authorizes extension up to R13 hard stop. Same PRP-1507 deviation note as R2-R9. | Claude Opus 4.7 |
| 2026-05-17 | R11 (PR #177 codex bot P2 on R10 head): **FXA-2289** step 3 sequencing bug — R10's quality gate ran AFTER `test:` commit, so rejected tests would land in branch history; re-dispatch would layer new tests on top of the bad commit rather than replace it. Fix: gate now runs on the un-committed worktree BEFORE commit; on gate fail, orchestrator cleans the worktree per step 1's Cleanup contract and re-dispatches (no `test:` commit produced for the rejected attempt). On gate pass, the `test:` commit follows. Same PRP-1507 deviation note as R2-R10. | Claude Opus 4.7 |
| 2026-05-17 | R12 (PR #177 codex bot P2 ×2 on R11 head): two follow-on findings — (1) **FXA-2289** step 3 rejection cleanup scope: R11's rejection path said "cleans worktree per step 1's Cleanup contract", but step 1's contract only covers OFF-test-path entries; rejected test files AT allowed test paths (e.g. vacuous test in `tests/test_foo.py`) remain dirty after rejection. Fix: rejection cleanup now explicitly restores `<test-writer-paths>` via `git restore --source=HEAD --staged --worktree -- <test-writer-paths>` plus `git reset HEAD -- <new-test-paths>` + `git clean -f -- <new-test-paths>` for new-test files not in HEAD. (2) **FXA-2289** step 8(a) additive-coverage unreachability: step 8(a) said "re-enter from step 1" but step 2's RED-must-fail gate unconditionally rejects passing tests; in the additive-coverage sub-case the test legitimately passes against current HEAD, so step 2 rejects and the documented additive-coverage branch becomes unreachable. Fix: step 8(a) now **bypasses step 2's RED gate** explicitly, going test-writer-dispatch → step 3 quality gate + commit → local-run classification (fails → dispatch implementer; passes → done with `test:` commit only). R-count = 12 / 13 hard stop per FXA-2276 Case B; one round remaining. Same PRP-1507 deviation note as R2-R11. | Claude Opus 4.7 |
| 2026-05-17 | R13 (PR #177 codex bot P2 on R12 head — HARD STOP round per FXA-2276 Case C): **FXA-2289** step 8(a) blanket-bypass of step 2's RED gate (R12 fix) was too permissive. Codex caught: if the test-writer accidentally writes a semantically wrong but non-vacuous test that passes against current HEAD, the flow commits it and skips the implementer, so the reviewer's edge-case fix request is marked done with no production change. Fix: step 8(a) now requires **pre-dispatch classification** — orchestrator MUST classify each Phase 8 case (a) iteration as either (a.i) `new-behavior-and-test` (default, step 2 RED gate applies) OR (a.ii) `coverage-only` (rare, step 2 bypassed, test expected to pass). For (a.i), passing tests are rejected as before. For (a.ii), if the test unexpectedly fails on current HEAD, escalate (misclassification: reviewer intended (a.i) but used (a.ii) language). The classification cannot be skipped. **R-count = 13 = hard stop per Case C**; no further fixes allowed in this PR. Same PRP-1507 deviation note as R2-R12. | Claude Opus 4.7 |
